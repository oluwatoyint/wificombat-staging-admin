...
const Teachers = () => {
    ...
    const { data, error, loading, refetch } = useFetch(
        "https://wificombat-staging-backend-production.up.railway.app/administration/get-users?role=teacher",
                ...
                <div className="mx-4 flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3">
                            <p className="text-[#101828] text-[19px]">Tutors</p>
                            <p className="text-[#056494] bg-[#E6F6FE] py-1 px-[8px] rounded-full">
                                {teachers?.count} tutors
                            </p>
                        </div>
                        <p className="text-[#475467] text-[14px]">Oversee all tutors activities</p>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <tbody>
                        ...
                        >
                            <td className="py-4 pl-4 flex items-center gap-3">
                                <img
                                    src={teacher?.profile_pic?.media ? teacher?.profile_pic?.media : '/assets/teacher-avatar.svg'}
                                    className="w-[40px] h-[40px] rounded-full object-cover"
                                    alt={teacher.first_name}
                                />
                                <div className='flex items-center gap-2'>
                                    <p className="font-medium text-[#1D2939]">{teacher.first_name ? teacher.first_name : 'N/A'}</p>
                                    <p className="font-medium text-[#1D2939]">{teacher.last_name ? teacher.last_name : 'N/A'}</p>
                                </div>
                            </td>
                            ...
                            {
                                teacher.last_login ?
                                <td className="py-4 px-4 capitalize">{ moment(teacher.last_login).fromNow() }</td>
                                :
                                <td className="py-4 px-4">N/A</td>
                            }
                            <td className="py-4 px-4">
                                <Link
                                    href={`/activity/${teacher.id}`}
