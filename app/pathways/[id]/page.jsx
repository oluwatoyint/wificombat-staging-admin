...
import { get, post, put, uploadFile } from '../../utils/axiosHelpers';
...
const PathwayInfo = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (selectedFile && selectedFile.size <= 16 * 1024 * 1024) { // 16MB limit
            ...
            if (uploadResponse?.success) {
                ...
                pathwayData.cover_image = uploadResponse.data
            } else {
    ...
    async function updatePathway(){
        ...
        const pathwayToUpdate = { ...pathwayData, cover_image:pathwayData.cover_image.id  }
        console.log(pathwayToUpdate);
        
        
        try {
            setLoading(true)
            const response = await put(`/course-pathways/${id}/`, pathwayToUpdate);
            console.log(response);
            setMsg("Pathway updated successfully.");
            setAlertType('success');
        } catch (error) {
    ...
    async function getPathwayInfo(){
        try {
            ...
            setUploadedFile(response.data.cover_image)
            
        } catch (error) {
            setMsg("An error occurred while fetching pathway details.");
            setAlertType('error');
            console.log(error);
        }finally{
            setLoading(false)
        ...
        <div className='w-[800px] py-5 px-4 border border-[#E5E5E6] rounded-[10px]'>
            ...
            </div>
            {
                loading ?
                    ...
                    :
                    <button onClick={updatePathway} className='bg-[#B1B1B4] mt-5 w-[150px] py-[13px] rounded-[10px] ml-auto block text-[#E5E5E6] font-[500] hover:bg-[#131314]'>Update Pathway</button>
            }
        </div>
    </div>
  )
