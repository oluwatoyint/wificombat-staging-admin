...
const AssignmentComponent = () => {
    ...
    async function getAllAssignments() {
        try {
            setLoading(true)
            const response = await get(`/assignments/get_all?course_id=${id}`)
    ...
    async function getAssignments(mdlId) {
        try {
            setLoading(true)
            const response = await get(`/assignments/get_all?module_id=${mdlId}`)
            console.log("Response Data", response.data);
    ...
    <div>
        ...
        </div>
        {
            ...
            <div className='text-[#656765] text-center flex flex-col justify-center items-center w-full mt-20'>
                <p>No assignments found for this course or module.</p>
        ...
        }
        {
            allAssignment?.map((assignment, index) => (
                <div key={index} className='border p-4 mb-3 flex items-center justify-between rounded-[10px]'>
                    <div>
                        <p className='mb-2 font-[500]'>{assignment?.title}</p>
                        {/* <p dangerouslySetInnerHTML={{ __html: assignment.description }} />
                        <p>{trimText(assignment.description, 150)}</p> */}
                    </div>
                    <GoChevronRight onClick={() => router.push(`/pathway/${path}/${id}/assignment/${assignment.id}`)} className='text-[20px] cursor-pointer'/>
                </div>
            ))
        }
        {
