...
const UpdateDiscount = () => {
    ...
    async function getDiscountData(){
        try {
            const response = await get(`/administration/discount-codes/${discountId}/`)
            console.log(response);
            setDiscountData({
                // courses: response.data.courses,
                is_active: response.data.is_active,
                max_uses: response.data.max_uses,
                percentage_off: response.data.percentage_off,
            ...
            // setSelectedCoursesIds(response.data.courses)
            // setSelectedCourses(response.data.courses)
        } catch (error) {
            console.log(error)
        }
    ...
    <div className="w-[82.5%] px-8 py-6 mx-auto relative top-[8.5rem] left-1/2 transform -translate-x-1/2">
        ...
        <div className='w-[800px] py-5 px-4 border border-[#E5E5E6] rounded-[10px]'>
            ...
            <div className='mt-5'>
                <div className='w-full relative'>
                    <p>Max Usage</p>
                    <div onClick={() => setDropDown(dropDown === "usage" ? "" : "usage")} className='border flex items-center justify-between py-3 mt-1 border-[#B1B1B4] rounded-[8px] w-full text-[#656765] px-2 cursor-pointer'>
                        <p className='text-[#656765]'>{discountData.max_uses ? discountData.max_uses : 'Select max usage'}</p>
                        <BiChevronDown className='text-[22px]' />
                    </div>
                    {
                        dropDown === 'usage' &&
                        <ul className='absolute z-10 top-[80px] bg-[#fff] rounded-[8px] w-full border border-[#B1B1B4] py-2'>
                            {usageArray?.map((useAge, index) => (
                                <li key={index} onClick={() => {
                                    setDiscountData(prev => ({
                                        ...prev,
                                        max_uses: useAge
                                    ...
                                    setDropDown(null);
                                }} className='py-1 px-2 cursor-pointer hover:bg-slate-200'>{useAge}</li>
                            ))}
                        </ul>
                    }
                </div>
